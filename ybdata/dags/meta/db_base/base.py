...
from . import DBError, NotSupportError, ParamError
...
class CRUD(RelationDBEngine, ABC):
    ...
    def _select(
        self, table: Table, condition: Optional[BooleanClauseList] = None
    ) -> ResultProxy:
    ...
    def _delete(self, table: Table, condition: BooleanClauseList) -> ResultProxy:
    ...
    def insert_one(
        self, table: str, data: Dict[str, Any], schema: Optional[str] = None
    ) -> None:
        """
        插入一条数据
    ...
    def insert_many(
        ...
        if not isinstance(data, abc.Sequence) or isinstance(data, str):
            raise ParamError(
                "insert_many 只支持含有多条数据的序列, 例如 list, tuple 等"
            )
    ...
    def read_sql(
        self, sql: str, chunksize: Optional[int] = None
    ) -> Union[pd.DataFrame, Iterator]:
        """
    ...
    def delete(
        self, table: str, condition: BooleanClauseList, schema: Optional[str] = None
    ):
        """
    ...
    def update(
        ...
        obj_tb = self._reflect_table(table, schema)
        return self._update(obj_tb, data, condition).rowcount
